 ...
 EM.skewCens.mixR <- try(
   function(cc, y, x, Abetas = NULL, sigma2 = NULL, shape = NULL, pii = NULL, nu=NULL, g = NULL, get.init = TRUE, criteria = TRUE, group = FALSE, family = "Normal", error = 0.00001, iter.max = 100, obs.prob= FALSE, kmeans.param = NULL, aitken = TRUE, IM=TRUE)
 )
 if(class(fit.arp) != "try-error"){
  #y:  data of size  n
...
# else obj.out <- obj.out[-length(obj.out)] 
 ...
 else{write.table(i,file="SIMerror.txt",sep=" ",row.names=F,col.names=F,append = TRUE)} # end if try 
